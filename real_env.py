...
class RealRobotEnv:
    ...
    def synced_callback(self, camera_1_image, tac1_data, tac2_data, xarm_eef):
        raw_obs_dict = {}
        ...
        raw_obs_dict['robot_state'] = xarm_eef_array
        raw_obs_dict['wrist_image'] = cv2.resize(cam1_img, (320, 240))[...,::-1]
        raw_obs_dict['tactile1'] = np.concatenate((mesh1, deform1), axis=-1)
        raw_obs_dict['tactile2'] = np.concatenate((mesh2, deform2), axis=-1)
...
if __name__ == "__main__":
    env = RealRobotEnv(n_obs_steps=2)
    rossub_thread = threading.Thread(target=env.ros_thread, daemon=True)
    rossub_thread.start()
    while True:
        start = time.time()
        obs = env.get_obs()