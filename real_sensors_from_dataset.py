...
class RealRobotEnv:
    ...
    def get_obs(self) -> Dict[str, np.ndarray]:
        ...
        for i in range(self.n_obs_steps):
            # obs = self.dataset[self.timestep+i]['obs']
            obs_dict = {}
            for key in obs.keys():
                obs_dict[key] = obs[key][i]#
                # print(f"obs_dict[{key}].shape is {obs_dict[key].shape}")
                if 'img' in key:
                    obs_dict[key] = obs_dict[key].permute(1,2,0)
    ...
    #     self.executed_actions.append(action[0:3])  
    
    def update_obs(self, action_chunk):
        plot_action = {
            'fact':[],
            'predict':[]
        }
        fact_action_chunk = self.dataset[self.timestep]['action'].numpy()
        # print(f"fact_action_chunk.shape is {fact_action_chunk.shape}")
        # print(f"action_chunk.shape is {action_chunk.shape}")
        # print(fact_action_chunk[:,0:3]*1000)
        # print(action_chunk[:,0:3]*1000)
        # print(self.dataset[self.timestep]['obs']['left_robot_tcp_pose'][0][0:3]*1000)
        # print(self.dataset[self.timestep]['obs']['left_robot_tcp_pose'][1][0:3]*1000)
        l1 = torch.mean(torch.abs(torch.Tensor(action_chunk[None,:,:9]) - torch.Tensor(fact_action_chunk[None,:,:9])))
        print(f"l1 loss is {l1.item()}, timestep is {self.timestep}")
        # print("===============")
    ...
    def save_plot_actions(self, save_path="data/outputs/vis_outputs/dp_plot_actions.pkl"):
