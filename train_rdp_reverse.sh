# #!/bin/bash
# 使用reverse版本的dataset `RealImageTactileLatentDiffusionDatasetReverse(RealImageTactileDatasetReverse)`, 不只预测当前和未来的action, 还预测过去的action
# 现在训练不起来, 因为ldm输出的是action latent, 实际负责action的是vae. 而vae的输入是完整的extended_obs, 因此我可能需要训练两个vae decoder, 一个正向, 一个反向
# 先训练dp吧
...
CUDA_VISIBLE_DEVICES=${GPU_ID} python train.py \
    ...
    task=real_${TASK_NAME}_image_gelsight_emb_at_24fps_reverse \
...
CUDA_VISIBLE_DEVICES=${GPU_ID} accelerate launch train.py \
    ...
    task=real_${TASK_NAME}_image_gelsight_emb_ldp_24fps_reverse \
    task.dataset_path=${DATASET_PATH} \
